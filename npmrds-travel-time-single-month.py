...
for year in analysis_year:
    ...
    for months in analysis_months:
        ...
        for x in range(0,len(time_of_day['TOD_Name'])):
            ...
            df_tod = df_current_spd[df_current_spd['measurement_tstamp'].dt.hour == time_of_day['start_time'][x]]
        ...
        if output_csv == 'yes':
            df_output.to_csv(os.path.join(output_directory, months + '_' + year + '_' + vehicles +'_tmc_'+str(int(input_percentile*100))+'th_percentile_speed.csv'),index=False)
